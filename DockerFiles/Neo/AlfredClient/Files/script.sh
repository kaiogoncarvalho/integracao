...
sed -i "s:\$PROJECT_PUBLIC_FOLDER:$(echo $PROJECT_PUBLIC_FOLDER):" /usr/local/${NGINX_VERSION}/conf/apps/alfred-client.conf
sed -i "s:\localhost:$(echo $SITE_HOST):" /usr/local/${NGINX_VERSION}/conf/apps/alfred-client.conf
